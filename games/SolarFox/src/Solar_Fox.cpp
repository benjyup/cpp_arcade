...
arcade::Solar_Fox::Solar_Fox(void *handle)
        : _shot(false), _objects(), _lib(NULL), _win(NULL), _map(), _score(0), _Solar_Fox(), _map_size({0, 0}), _handle(handle),
          ...
          _posFin(2), _time(0), _lastKc(arcade::CommandType::GO_RIGHT)
{
...
void arcade::Solar_Fox::_powerUp() {
    ...
    for (auto &it : _Solar_Fox.objsPowerUp) {
        if (_dir == 1) {
            if ((it->getPosition().getX() == p.x || it->getPosition().getX() == p.x - 1 ||
        ...
        if (_shot == true) {
            ...
            for (auto &it : _Solar_Fox.objsShoot)
                if (it->getPosition().getX() == p.x && it->getPosition().getY() == p.y) {
                    _win->moveObject(it,
                                     {_Solar_Fox.body[S_Solar_Fox_HEAD].x, _Solar_Fox.body[S_Solar_Fox_HEAD].y - 2});
                    _last_pos.x = p.x;
                    _last_pos.y = p.y - 2;
                }
            _dir = 4;
    ...
    if (_tmp == 10) {
        ...
        if (_shot == true) {
            ...
            for (auto &it : _Solar_Fox.objsShoot)
                if (it->getPosition().getX() == p.x && it->getPosition().getY() == p.y) {
                    _win->moveObject(it,
                                     {_Solar_Fox.body[S_Solar_Fox_HEAD].x - 2, _Solar_Fox.body[S_Solar_Fox_HEAD].y});
                    _last_pos.x = p.x - 2;
                    _last_pos.y = p.y;
                }
            _dir = 2;
